"""CTGAN module adapted from DRL_DGM.
...
class CTGAN(BaseSynthesizer):
    ...
    def _gumbel_softmax(logits, tau=1, hard=False, eps=1e-10, dim=-1):
        ...
        The retry loop below follows the upstream CTGAN implementation's
    ...
    def get_sets_constraints(self, label_ordering_choice, constraints_file):
        ...
        ordering = set_ordering(self.args.use_case, ordering_template, label_ordering_choice, 'ctgan')
    ...
    def fit(self, args, train_data, discrete_columns=(), epochs=None):
        ...
        for epoch in range(epochs):
            ...
            epoch_loss_g = float(loss_g_running) / steps_per_epoch
            epoch_loss_d_syn = float(loss_d_syn_running) / steps_per_epoch
            epoch_loss_d_real = float(loss_d_real_running) / steps_per_epoch
            epoch_loss_d = float(loss_d_running) / steps_per_epoch
            self.loss_history.append({
                'epoch': epoch + 1,
                'loss_g': epoch_loss_g,
                'loss_d': epoch_loss_d,
                'loss_d_syn': epoch_loss_d_syn,
    ...
    def sample(self, n, condition_column=None, condition_value=None):
        ...
        if self._version == "constrained" or self._version == "postprocessing":
            ...
            ordering, sets_of_constr = self._pick_ordering()
            inverse = correct_preds(inverse, ordering, sets_of_constr)
