...
INFINITY = torch.inf
# Sentinel used to replace ``+/- inf`` values in the final correction tensor.
# ``correct_preds`` materialises a device-matching copy at call time so this
# module-level CPU tensor never leaks into a GPU computation.
...
def get_lb_from_one_constraint(x: Variable, constraint: Constraint, preds: torch.Tensor, epsilon=1e-12):
    ...
    if len(complement_body_atoms) == 0:
        eval_body = torch.zeros(preds.shape[0], device=preds.device, dtype=preds.dtype)  # shape (B,)
    else:
...
def correct_preds(preds: torch.Tensor, ordering: List[Variable], sets_of_constr: {Variable: List[Constraint]}):
    ...
    # Materialise the +/-inf sentinel on the same device as ``corrected_preds``
    # so the final clamps stay on GPU when one is available.
    infinity_sentinel = INFINITY_NP.to(device=corrected_preds.device, dtype=corrected_preds.dtype)
    corrected_preds = torch.where(corrected_preds == torch.inf, infinity_sentinel, corrected_preds)
    corrected_preds = torch.where(corrected_preds == -torch.inf, -infinity_sentinel, corrected_preds)
