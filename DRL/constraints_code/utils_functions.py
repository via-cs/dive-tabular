...
def eval_atoms_list(atoms_list: List['Atom'], preds: torch.Tensor, reduction='sum'):
    ...
    if evaluated_atoms == []:
        # The empty-body fallback has to land on the same device/dtype as
        # ``preds``; otherwise a stray CPU zero leaks through and every
        # downstream op (``torch.where``, ``torch.cat``) raises a
        # cross-device error when the caller is on GPU.
        return torch.zeros(preds.shape[0], device=preds.device, dtype=preds.dtype)
