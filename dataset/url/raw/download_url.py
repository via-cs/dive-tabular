"""Download the Web Page Phishing Detection CSV from Kaggle.

Examples:
    uv run python dataset/url/raw/download_url.py
    uv run python dataset/url/raw/download_url.py /tmp/url-dataset
    uv run python dataset/url/raw/download_url.py /tmp/url-source.csv --force

The output may be either a directory or an explicit CSV filename. The default
destination is ``dataset/url/raw/dataset_phishing.csv``.
"""
...
KAGGLE_FILE = "dataset_phishing.csv"
EXPECTED_ROWS = 11_430
EXPECTED_COLUMN_COUNT = 89
REQUIRED_COLUMNS = {
    "url",
    "length_url",
    "length_hostname",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "dataset/url/raw/dataset_phishing.csv."
        ),
    )
    parser.add_argument(
        "--force",
...
def validate_csv(path: Path) -> None:
    """Require the complete Kaggle phishing source shape and key columns."""
    ...
    if len(header) != EXPECTED_COLUMN_COUNT:
        raise RuntimeError(
            f"Downloaded CSV has {len(header)} columns; "
...
def download(destination: Path, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(
        prefix=".web-page-phishing-", dir=destination.parent
